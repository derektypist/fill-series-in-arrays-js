...
let squares = (s,e) => Array(e+1-s).fill(s).map((e,i) => i**2);
let cubes =  (s,e) => Array(e+1-s).fill(s).map((e,i) => i**3);
let random = Array(10).fill(0).map((i) => Math.floor(Math.random()*100)+1);
let multOf3and5 = (s,e) => Array(e+1-s).fill(false).map((i) => i%3==0 || i%5 == 0 ? true : false);

// Build the text
txt += `Natural Numbers: ${natural(1,10)} <br>`;
txt += `Square Numbers: ${squares(2,11)} <br>`;
txt += `Cube Numbers: ${cubes(2,11)} <br>`;
